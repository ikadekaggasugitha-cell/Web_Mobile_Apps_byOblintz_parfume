...
// swap to the API port — open the admin app on any IP and the API resolves to
// the same IP automatically. On the server (SSR/build) we hit localhost, since
// the Next server always runs on the same machine as the API.
